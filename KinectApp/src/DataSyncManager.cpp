...
void DataSyncManager::loadFromJsonRef( ofxJSONElement data )
{
	...
	for ( int i = 0 ; i < numSpectrums ; i++ ) 
	{
		...
		string wavelength =					data[ "spectrums" ][i]["wavelength"].asString() ; 
		string primaryIons =				data[ "spectrums" ][i]["primaryIons"].asString() ; 
		...
		atmosphericImageData.push_back( new AtmosphericImageData(	label , description , location , wavelength , primaryIons , temperatureKelvin, temperatureFahrenheight , assetUrl  ) ) ; 
